...
export function generateSQLString(
  ...
  maxSizes: number,
  forceSingle: boolean,
  couleurs: { nom: string; mode: string }[],
  ...
  couleurs.forEach((c, ci) => {
    sql += `INSERT INTO colors(id, nom, mode) VALUES (${ci}, '${esc(c.nom)}', '${esc(c.mode)}');\n`;
  ...
  lines.forEach(line => {
    ...
    if (trimmed.startsWith('INSERT INTO packing_config')) {
      ...
      if (valGroup) {
        const parts = parseSqlValues(valGroup[1]);
        if (parts.length >= 11) {
          const color_id = parseInt(parts[0]);
          const size_index = parseInt(parts[1]);
          const name = parts[2].replace(/^'|'$/g, '');
          const qty = parseInt(parts[3]) || 0;
          const cap = parseInt(parts[4]) || 25;
          const w_piece = parseFloat(parts[5]) || 0.25;
          const w_carton = parseFloat(parts[6]) || 0.8;
          const dim_L = parseFloat(parts[7]) || 61;
          const dim_l = parseFloat(parts[8]) || 41;
          const dim_h = parseFloat(parts[9]) || 30;
          const sku = parts[10] ? parts[10].replace(/^'|'$/g, '') : '';

          if (!sizes[color_id]) sizes[color_id] = {};
          sizes[color_id][size_index] = { name, qty, cap, w_piece, w_carton, dim_L, dim_l, dim_h, sku };
        }
      }
    }
  });
...
export async function exportToExcel(
  ...
  allResults.forEach((res, ci) => {
    ...
    // Column widths
    ws.columns = [
      { width: 16 }, { width: 18 }, { width: 22 },
      ...tailles.map(() => ({ width: 9 })),
      { width: 11 }, { width: 11 }, { width: 14 }, { width: 16 }, { width: 17 }, { width: 12 }
    ];
    ...
    const hdrVals = ['PACKING N°', 'COLOR / COULEUR', 'SKU', ...tailles, 'PCS/CTN', 'NB CTN', 'TOTAL QTY', 'NET WEIGHT\n(kg)', 'GROSS WEIGHT\n(kg)', 'CBM (m³)'];
    hdrVals.forEach((val, index) => {
      ...
      if (index === 0 || index === 1) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.navyBg } };
      else if (index === 2) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.greenBg } };
      else if (index >= 3 && index <= 2 + tailles.length) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.blueBg } };
      else if (index === nc - 3) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.tealBg } };
      else if (index === nc - 2) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.redBg } };
      else cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.navyBg } };
    });
    hdrRow.height = 32;
    const hdrRi = ri;
    ri++;
    ...
    rows.forEach((r, rIdx) => {
      ...
      vals.forEach((val, valIdx) => {
        ...
        if (valIdx === 0) cell.font = { color: { argb: 'FF' + colorsHex.navyBg }, size: 10, bold: true };
        else if (valIdx === 1) cell.font = { color: { argb: '000000' }, size: 10, bold: true };
        else if (valIdx === 2) cell.font = { color: { argb: 'FF' + colorsHex.skuFg }, size: 10 };
        else if (valIdx === nc - 3) cell.font = { color: { argb: 'FF' + colorsHex.netFg }, size: 10, bold: true };
        else if (valIdx === nc - 2) cell.font = { color: { argb: 'FF' + colorsHex.grossFg }, size: 10, bold: true };
        else cell.font = { color: { argb: '333333' }, size: 10 };
      });
    ...
    ws.views = [{ state: 'frozen', xSplit: 3, ySplit: hdrRi, activeCell: ws.getRow(hdrRi + 1).getCell(4).address }];
  });
  ...
  if (allResults.length > 1) {
    ...
    const ncComb = combinedSizes.length + 9;
    ...
    const hdrVals = ['PACKING N°', 'COLOR / COULEUR', 'SKU', ...combinedSizes, 'PCS/CTN', 'NB CTN', 'TOTAL QTY', 'NET WEIGHT\n(kg)', 'GROSS WEIGHT\n(kg)', 'CBM (m³)'];
    hdrVals.forEach((val, index) => {
      const cell = hdrRow.getCell(index + 1);
      cell.value = val;
      cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
      ...
      else if (index >= 3 && index <= 2 + combinedSizes.length) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.blueBg } };
      else if (index === ncComb - 3) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.tealBg } };
      else if (index === ncComb - 2) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.redBg } };
      else cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + colorsHex.navyBg } };
    });
    hdrRow.height = 32;
    const hdrRiComb = ri;
    ...
    allResults.forEach((res, ci) => {
      ...
      rows.forEach(r => {
        ...
        const rangeStr = `${globalCartonNum}-${globalCartonNum + r.nbr - 1}`;
        ...
        vals.forEach((val, valIdx) => {
          ...
          else if (valIdx === ncComb - 3) cell.font = { color: { argb: 'FF' + colorsHex.netFg }, size: 10, bold: true };
          else if (valIdx === ncComb - 2) cell.font = { color: { argb: 'FF' + colorsHex.grossFg }, size: 10, bold: true };
          else cell.font = { color: { argb: '333333' }, size: 10 };
        });
    ...
    wsComb.views = [{ state: 'frozen', xSplit: 3, ySplit: hdrRiComb, activeCell: wsComb.getRow(hdrRiComb + 1).getCell(4).address }];
  }
