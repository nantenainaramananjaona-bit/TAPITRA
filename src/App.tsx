...
export default function App() {
  ...
  const handleResetScreen = () => {
    if (!window.confirm('Réinitialiser toutes les données de la page ?')) return;
  ...
  const handleExcelExport = async () => {
    try {
      ...
      await exportToExcel(activeResults, meta, sizesInputsMapping);
    } catch (err: any) {
  ...
  const handleExportSQL = () => {
    ...
    const sqlStr = generateSQLString(
      ...
      maxSizesPerBox,
      forceSingleCarton,
      colors.map(c => ({ nom: c.nom, mode: c.mode })),
      inputSizesMapping,
      activeResults
    );
        ...
        parsed.colors.forEach((colRow) => {
          ...
          sizesListNames.forEach((name, idx) => {
            ...
            sizesDict[name] = {
              ...
              wPiece: raw.wPiece,
              wCarton: raw.wCarton,
              cbmUnit: (raw.dimL * raw.diml * raw.dimH) / 1000000,
              dimL: raw.dimL,
              diml: raw.diml,
              dimH: raw.dimH,
              sku: raw.sku || ''
    ...
    <div className={`min-h-screen font-sans ${darkMode ? 'bg-[#0f1117] text-[#e8eaf0]' : 'bg-[#f0f2f8] text-[#1a1d2e]'} transition-colors duration-300 pb-12`}>
      ...
      <header className={`max-w-7xl mx-auto px-4 py-4 md:py-6 flex flex-wrap items-center justify-between gap-4 border-b ${darkMode ? 'border-slate-800' : 'border-slate-200'} mb-6 print:hidden`}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-r from-[#4f8ef7] to-[#9b72f5] rounded-xl flex items-center justify-center shadow-md">
            <Package className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-lg font-bold tracking-tight">Packing List Pro</h1>
            <p className="text-[10px] uppercase font-mono tracking-widest text-slate-500">LOGISTICS OFFICER ENVIRONMENT</p>
          </div>
        </div>

        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-xs font-mono bg-blue-500/10 border border-blue-500/20 text-blue-500 px-2.5 py-1 rounded-full tracking-wider font-semibold">
            v10.0 — REACT
          </span>

          <button
            onClick={() => setIsMajBsdOpen(true)}
            className="px-3.5 py-2 whitespace-nowrap bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 font-mono text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 cursor-pointer"
          >
      ...
      <main className="max-w-7xl mx-auto px-4 space-y-6 print:px-0 pb-16">
        ...
        <div className="sticky top-2 z-50 print:hidden drop-shadow-md">
          <div className={`p-4 rounded-xl border flex flex-col md:flex-row items-center justify-between gap-4 transition-all duration-300 ${
            darkMode ? 'bg-[#0a162f]/95 border-[#1d3461] shadow-[#000000]/50' : 'bg-[#0d2149]/95 border-[#1e3c75] text-white shadow-blue-900/20'
          } backdrop-blur-md`}>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <div>
                <h2 className="text-xs font-mono font-bold uppercase tracking-wider text-blue-300">
                  CONSOLE DE PILOTAGE
                </h2>
                <div className="text-[11px] font-mono font-bold mt-0.5">
                  {hasGenerated ? (
                    <span className="text-emerald-400">PACKING LIST DISPONIBLE</span>
                  ) : (
        ...
        <div className="grid grid-cols-1 gap-6 print:hidden">
          ...
          <div className={`rounded-xl border p-5 ${darkMode ? 'bg-[#161a23] border-slate-800' : 'bg-white border-slate-200'} space-y-4`}>
          ...
          <div className={`rounded-xl border p-5 ${darkMode ? 'bg-[#161a23] border-slate-800' : 'bg-white border-slate-200'} space-y-4`}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/10 p-2 border border-slate-800 rounded-xl">
                ...
                {colors.map((c, idx) => (
                  <button
                    ...
                    className={`px-3.5 py-2 rounded-lg text-xs font-mono font-bold transition-all flex items-center gap-2 cursor-pointer ${
                      activeColorIdx === idx
                        ? 'bg-[#1f2430] border border-blue-500/50 text-blue-400 font-extrabold shadow-md'
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 text-blue-400 font-bold rounded-lg text-xs transition-all cursor-pointer flex items-center gap-1"
                >
                  <Plus className="w-3.5 h-3.5" /> ＋ Couleur
                </button>
                <button
                  onClick={handleRemoveActiveColorTab}
                  className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 font-bold rounded-lg text-xs transition-all cursor-pointer flex items-center gap-1"
                >
            ...
            {colors[activeColorIdx] && (
              <div className="space-y-4 pt-1 animate-fadeIn">
                <div className="flex flex-wrap items-center gap-4 justify-between bg-[#1f2430]/30 border border-slate-800/50 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      className="text-sm font-mono font-bold uppercase py-1 border-b border-dashed border-slate-700 focus:border-blue-500 bg-transparent text-white focus:outline-none"
                      placeholder="NOM COULEUR"
                    />
                    <span className="text-[10px] text-slate-500 font-mono">(Saisissez pour renommer)</span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition-all cursor-pointer ${
                        colors[activeColorIdx].mode === 'inherit'
                          ? 'bg-amber-500/10 border border-amber-500 text-amber-500 font-black'
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 text-blue-400 font-mono text-xs font-bold rounded-lg cursor-pointer transition-all flex items-center gap-1"
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 font-mono text-xs font-bold rounded-lg cursor-pointer transition-all flex items-center gap-1"
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-800 bg-slate-900/10">
                  <table className="w-full text-xs text-center border-collapse">
                    <thead>
                      <tr className="bg-slate-900/60 text-slate-400 font-mono font-bold border-b border-slate-800">
                        <th className="py-3 px-4 text-left border-r border-slate-800 font-sans tracking-wide">
                          PARAMÈTRE DE COLISAGE
                        </th>
                        {colors[activeColorIdx].tailles.map((sz, idx) => (
                          <th key={idx} className="border-r border-slate-800 min-w-28 p-1">
                            <input
                              type="text"
                              value={sz}
                              onChange={(e) => handleSizeHeaderChange(idx, e.target.value)}
                              className="w-full text-center bg-[#1f2430] border border-transparent focus:border-blue-500 hover:border-slate-800 font-mono font-semibold text-white px-2 py-1 rounded-md focus:outline-none uppercase"
                            />
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-slate-800/70 font-mono">
                      {/* Row 1: QTY Totale */}
                      <tr>
                        <td className="py-2 px-4 text-left font-sans font-semibold border-r border-slate-800 bg-[#222636]/10">
                          Quantité Totale à Emballer
                        </td>
                        {colors[activeColorIdx].tailles.map((sz) => (
                          <td key={sz} className="p-1 border-r border-slate-800">
                            <input
                              type="number"
                              min="0"
                              value={colors[activeColorIdx].sizes[sz]?.qtyTot || ''}
                              onChange={(e) => handleUpdateSizeCell(sz, 'qtyTot', e.target.value)}
                              className={`w-full text-center py-1.5 font-bold rounded-md bg-[#222636] text-white border border-slate-800 focus:border-blue-500 focus:outline-none`}
                              placeholder="0"
                            />
                          </td>
                        ))}
                        ...
                        {colors[activeColorIdx].tailles.map((sz) => (
                          <td key={sz} className="p-1 border-r border-slate-800">
                            <input
                              ...
                              className="w-full text-center py-1.5 text-slate-300 font-bold bg-[#222636] border border-slate-800 focus:border-blue-500 focus:outline-none rounded-md"
                              placeholder="25"
                            />
                          </td>
                        ))}
                      ...
                      {/* Row 3: SKU per size */}
                      <tr>
                        <td className="py-2 px-4 text-left font-sans font-semibold border-r border-slate-800 bg-[#222636]/10 text-emerald-400">
                        ...
                        {colors[activeColorIdx].tailles.map((sz) => (
                          <td key={sz} className="p-1 border-r border-slate-800">
                            <input
                              ...
                              className="w-full text-center py-1 bg-[#222636] border border-slate-800 focus:border-blue-500 focus:outline-none rounded-md text-[11px] text-emerald-300 uppercase font-semibold"
                              placeholder="SKU"
                            />
                          </td>
                        ))}
                      ...
                      {/* Row 4: Config Button */}
                      <tr>
                        <td className="py-2.5 px-4 text-left font-sans font-semibold border-r border-slate-800 bg-[#222636]/10">
                          Configure Gabarit (Poids/Dimensions)
                        </td>
                        {colors[activeColorIdx].tailles.map((sz) => {
                          const config = colors[activeColorIdx].sizes[sz];
                          return (
                            <td key={sz} className="p-1.5 border-r border-slate-800">
                              <button
                                onClick={() => openBoxDetailsModal(sz, config)}
                                className="px-3.5 py-1.5 bg-[#1a1d27] hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700/60 rounded-md font-mono text-xs w-full cursor-pointer transition-colors"
                              >
                                ✏️ Éditer
                              </button>
                            </td>
          ...
          {/* TEMPLATE MODELS DRAWER REMOVED */}
        </div>
                    ...
                    <table className="w-full text-xs text-center border-collapse">
                      <thead>
                        <tr className="bg-slate-900 border-b border-slate-800 font-mono font-bold text-[10px] text-slate-400">
                          {printColumns.ctn && <th className="py-2.5 px-3 uppercase text-left border-r border-slate-800 col-ctn-index">PACKING N°</th>}
                          {printColumns.color && <th className="px-2 border-r border-slate-800 col-color-lbl">COULEUR</th>}
                          {printColumns.sku && <th className="px-3 border-r border-slate-800 col-sku-lbl bg-emerald-500/5 text-emerald-400">SKU</th>}
                      ...
                      <tbody className="divide-y divide-slate-800/60 font-mono font-medium">
                        {res.rows.map((row, rIdx) => (
                          <tr key={rIdx} className="hover:bg-slate-800/40 divide-x divide-slate-800/40">
                            {printColumns.ctn && <td className="py-2 px-3 text-left font-bold text-slate-350 col-ctn-index">{row.cartonRange}</td>}
                            {printColumns.color && <td className="px-2 font-bold col-color-lbl" style={{ color: res.color }}>{res.nom}</td>}
                            {printColumns.sku && <td className="px-3 truncate max-w-28 text-[11px] text-emerald-400 font-semibold col-sku-lbl">{row.skus.join('/') || '—'}</td>}
                            {printColumns.sizes && activeColorSizes.map(t => (
                        ...
                        <tr className="bg-[#222636]/30 font-bold border-t border-slate-700 divide-x divide-slate-800">
                          {printColumns.ctn && <td className="py-2.5 px-3 text-left col-ctn-index">TOTALE</td>}
                          {printColumns.color && <td className="px-2 text-slate-300 col-color-lbl">{res.nom}</td>}
                          {printColumns.sku && <td className="px-3 col-sku-lbl">—</td>}
                          {printColumns.sizes && activeColorSizes.map(t => (
                  ...
                  <table className="w-full text-xs text-center border-collapse">
                    <thead>
                      <tr className="bg-slate-900 border-b border-slate-800 font-mono font-bold text-[10px] text-slate-400">
                        {printColumns.ctn && <th className="py-2.5 px-3 uppercase text-left border-r border-slate-800 col-ctn-index">PACKING N°</th>}
                        {printColumns.color && <th className="px-2 border-r border-slate-800 col-color-lbl">COULEUR</th>}
                        {printColumns.sku && <th className="px-3 border-r border-slate-800 bg-emerald-500/5 text-emerald-405 col-sku-lbl">SKU</th>}
                    ...
                    <tbody className="divide-y divide-slate-800/60 font-mono text-left">
                      {(() => {
                        ...
                        return results.map((res, ci) => {
                          ...
                          return res.rows.map((row, rIdx) => {
                            const rangeStr = `${seqNum}-${seqNum + row.nbr - 1}`;
                            seqNum += row.nbr;
                            rowCount++;
                            return (
                              <tr
                              ...
                              >
                                {printColumns.ctn && <td className="py-2 px-3 text-left font-bold text-slate-100 col-ctn-index">{rangeStr}</td>}
                                {printColumns.color && <td className="px-2 font-bold col-color-lbl" style={{ color: res.color }}>{res.nom}</td>}
                                {printColumns.sku && <td className="px-3 truncate max-w-28 text-[11px] text-emerald-500 col-sku-lbl">{row.skus.join('/') || '—'}</td>}
                                {printColumns.sizes && summaryUniqueSizes.map(t => (
                      ...
                      <tr className="bg-[#222636]/40 font-bold border-t border-slate-700 divide-x divide-slate-800">
                        {printColumns.ctn && <td className="py-2.5 px-3 text-left col-ctn-index">GRAND TOTAL</td>}
